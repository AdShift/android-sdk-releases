...
- **Twenty `-keep` rules you were told to copy are gone, because they never did anything** — the README asked you to add rules for `repeackage.com.*`, naming a top-level package that does not exist in this SDK; our copies of those classes have always lived under `com.adshift.sdk`, which line one of the shipped rules already keeps in full. If you added them by hand, deleting them changes nothing. If you kept them, they stay harmless.
